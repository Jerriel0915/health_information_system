<template>
  <div class="page-container">
    ...
    <el-row :gutter="16" class="chart-row">
      <el-col :span="12"><el-card shadow="hover"><template #header>费用构成<el-button text size="small" style="float: right;" @click="renderCompositionChart">刷新</el-button></template>

<script setup>
import { ref, reactive, onMounted, onBeforeUnmount } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import * as echarts from 'echarts'
import { listCost, updateCost, delCost, getCostSummary, getCostComposition, getCostTrend } from '@/api/system/cost'
...
const loadStats = async () => {
  try {
    ...
    stats.value = [
      { title: '费用总额', value: d.totalCost || d.totalAmount || 0, unit: '万元' },
      { title: '医保支付', value: d.insuranceAmount || d.insuranceCost || 0, unit: '万元' },
      { title: '自费金额', value: d.selfPayAmount || d.selfPayCost || 0, unit: '万元' },
      { title: '次均费用', value: d.avgCost || d.averageCost || 0, unit: '元' }
    ]
  } catch(e) { stats.value = [{title:'费用总额',value:0,unit:'万元'},{title:'医保支付',value:0,unit:'万元'},{title:'自费金额',value:0,unit:'万元'},{title:'次均费用',value:0,unit:'元'}] }
}

const chartRefs = { composition: ref(null), trend: ref(null) }
const charts = {}
...
const renderCharts = {
  composition: async () => {
    try {
      ...
      if (!chartRefs.composition.value) return
      if (charts.composition) charts.composition.dispose()
      charts.composition = echarts.init(chartRefs.composition.value)
      const list = Array.isArray(data) ? data : (data.records || data.list || [])
      charts.composition.setOption({
  ...
  trend: async () => {
    try {
      ...
      const values = list.map(d => d.value || d.count || d.totalCost || d.amount || 0)
      charts.trend.setOption({
        tooltip: { trigger: 'axis' }, xAxis: { type: 'category', data: years.length ? years : ['暂无'], name: '年份' },
        yAxis: { type: 'value', name: '费用（万元）' },
        series: [{ type: 'line', data: values.length ? values : [0], smooth: true, lineStyle: { color: '#F56C6C', width: 3 }, areaStyle: { opacity: 0.3 } }]
      })
    } catch(e) {}
  }
}
...
const getList = async () => { loading.value = true; try { const res = await listCost(queryParams); dataList.value = res.rows || res.records || res.list || []; total.value = res.total || dataList.value.length } catch(e) { dataList.value = []; total.value = 0 }; loading.value = false }
...
const submitForm = async () => { formRef.value?.validate(async val => { if (!val) return; try { if (form.value.id) await updateCost(form.value); else await addCost(form.value); ElMessage.success("操作成功"); dialogVisible.value = false; getList() } catch(e) { ElMessage.error("操作失败") } }) }
const handleDelete = (row) => { const id = row?.id || ids.value.join(","); ElMessageBox.confirm("确认删除？").then(async () => { try { await delCost(id); ElMessage.success("删除成功"); getList() } catch(e) { ElMessage.error("删除失败") } }).catch(() => {}) }
